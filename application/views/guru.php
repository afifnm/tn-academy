...
<div class="grid grid-cols-6 mt-5">
	<div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
		<button class="btn btn-primary shadow-md mr-2" data-tw-toggle="modal" data-tw-target="#tambahGuru">Tambah Data Guru</button>
		<!-- BEGIN: Modal Add User -->
		<div id="tambahGuru" class="modal" tabindex="-1" aria-hidden="true">
					...
					<form action="<?=base_url('admin/guru/add')?>" method="post">
						...
						<div class="modal-body ">
							<div class="">
								...
								<input id="validation-form-1" type="text" class="form-control" placeholder="Nama Guru"
		...
		<table class="table table-report -mt-2" id="myTable">
			<thead>
				<tr>
					...
					<th class="table-report__action text-center whitespace-nowrap">NAMA GURU</th>
					<th class="table-report__action text-center whitespace-nowrap">ACTIONS</th>
				</tr>
			</thead>
			<tbody>
				<?php $no=1+ ($offset ?? 0); foreach ($guru as $gu) {?>
				<tr class="intro-x">
					<td class="w-10"><?= $no ?></td>
					<td class="table-report__action text-center"><?= $gu['nama_guru'] ?></td>
					<td class="table-report__action w-60">
						<div class="flex justify-center items-center">
							<button class="flex items-center mr-3 text-warning" data-tw-toggle="modal"
								data-tw-target="#edit<?=$gu['id_guru']?>">
                                ...
                                <form action="<?=base_url('admin/guru/edit')?>" method="post">
                                    <div class="modal-body ">
                                        ...
                                        <div class="">
                                            <label for="modal-form-1" class="form-label">Nama Guru</label>
                                            <input id="validation-form-1" type="text" class="form-control" placeholder="Nama Guru"
                                                name="nama_guru" value="<?= $gu['nama_guru'] ?>" required>
                                        </div>
                                    </div>
