<div class="p-6">
    ...
    <div class="intro-y box p-5">
        <h2 class="text-lg font-medium mb-4">Filter Enroll Siswa</h2>
        <form method="GET" action="<?= base_url('admin/enrollsiswa/filter') ?>" class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="form-label">Tahun Ajaran</label>
                <select name="id_ta" class="form-control">
                    <option value="">-- Pilih Tahun Ajaran --</option>
                    <?php foreach ($tahun_ajaran as $ta): ?>
                        <option value="<?= $ta['id_ta']; ?>" <?= isset($filter['id_ta']) && $filter['id_ta'] == $ta['id_ta'] ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <select name="id_kelas" class="form-control">
                    ...
                    <?php foreach ($kelas as $k): ?>
                        <option value="<?= $k['id_kelas']; ?>" <?= isset($filter['id_kelas']) && $filter['id_kelas'] == $k['id_kelas'] ? 'selected' : '' ?>>
                            <?= $k['nama_kelas'] ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <table  id="example1" class="table table-auto display datatable w-full">
                ...
                <tbody>
                    <?php if (!empty($not_enrolled)): ?>
                        <?php $no = 1; foreach ($not_enrolled as $s): ?>
                            <tr>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="4" class="text-center">Semua siswa sudah enroll.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        <button type="submit" class="btn btn-success mt-3">Enroll Siswa Terpilih</button>
    </form>
</div>
